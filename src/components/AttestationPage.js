...
import backgroundImg from '../images/certif.png';
    ...
    <>
      <CertificateContainer>
        ...
        <CertificateCard className="certificate-card">
          ...
          <CertificateTitle>ATTESTATION DE PARTICIPATION</CertificateTitle>
          <CertificateText>Cette attestation est décerné à :</CertificateText>
          <CertificateName>{enseignant.nom} {enseignant.prenom}</CertificateName>
          <CertificateText>En reconnaissance de sa participation active à la formation intitulée :</CertificateText>
          <CourseName>{formationTitle}</CourseName>
          <DateText>Délivré le {new Date().toLocaleDateString()}</DateText>
        </CertificateCard>
      </CertificateContainer>
      <DownloadButton onClick={() => window.print()}>Télécharger l'attestation</DownloadButton>
    </>
  );
};
