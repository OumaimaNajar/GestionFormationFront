...
const loginUser = async (credentials) => {
  try {
    ...
    if (response.data.status === 'SUCCESS') {
      ...
      localStorage.setItem('userDepartment', response.data.departement || 'Département Non Disponible'); // Assurez-vous que 'departement' est correct
      return true;
    } else {
      return false;
    }
...
export const fetchParticipantsByFormation = async (idFormation) => {
  try {
      const response = await api.get(`/getEnseignantsByFormation/${idFormation}`);
      return response.data;
  } catch (error) {
...
// Générer des attestations pour un enseignant
export const genererAttestationsPourEnseignant = async (idEnseignant) => {
  try {
      const response = await api.post(`/genererPourEnseignant/${idEnseignant}`);
      return response;
  } catch (error) {
      throw new Error(`Erreur lors de la génération des attestations: ${error.message}`);
  }
};

// Télécharger une attestation au format PDF
export const telechargerAttestationPourEnseignant = async (idAttestation) => {
  try {
      const response = await api.get(`/telecharger/${idAttestation}`, {
          responseType: 'arraybuffer'
      });
      return response;
  } catch (error) {
      throw new Error(`Erreur lors du téléchargement de l'attestation: ${error.message}`);
  }
};
...
// Fonction pour récupérer les formations par ID d'enseignant
export const getFormationsByEnseignantId = async (idEnseignant) => {
  try {
    const response = await api.get(`/getFormationsByEnseignantId/${idEnseignant}`);
    return response.data;
  } catch (error) {
...
export const fetchEnseignantDetails = async (idEnseignant) => {
  try {
    const response = await api.get(`/getEnseignant/${idEnseignant}`);
    return response.data;
  } catch (error) {
